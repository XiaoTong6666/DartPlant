...
bool BuildGeneratedRootBindings(DispatchFrame* frame) {
    ...
    for (size_t parameter_index = 0; parameter_index < parameters->size(); ++parameter_index) {
        ...
        if (parameter.location.count == 0 && layout.closure_signature.has_value() &&
            frame->invocation.closure_argument_mapping_valid &&
            parameter_index < layout.closure_signature->formals.size() &&
            !layout.closure_signature->formals[parameter_index].is_required) {
            // Omitted optional positional/named formals have no physical slot
            // in this call. They remain visible through the invocation API but
            // must not be presented to the VM root lease.
            continue;
        }
...
#if defined(__ANDROID__) && defined(DARTPLANT_EXCEPTION_UNWIND_LOGGING)
        dartplant::AndroidLogPrint(
            ANDROID_LOG_INFO, "ExceptionUnwind", "depth=%u by_fp=%u by_sp=%u", stack->depth,
            static_cast<unsigned>(unwound_by_fp), static_cast<unsigned>(unwound_by_sp));
#endif
