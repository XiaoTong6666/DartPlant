...
uint64_t CapabilityForEvidence(DartPlantVmCapabilityEvidenceKind kind) {
    ...
    case DARTPLANT_VM_EVIDENCE_EXCEPTION_BRIDGE:
        return DARTPLANT_VM_CAP_EXCEPTION_BRIDGE_LAYOUT;
    }
    return 0;
}

size_t CapabilityIndex(uint64_t capability) {
    if (capability == 0 || (capability & (capability - 1)) != 0) {
...
void ResetAbiBindingLocked(DartPlantVmAdapter* adapter) {
    ...
    for (auto& binding : adapter->abi_binding.capabilities) binding = {};
    adapter->abi_binding.artifact_generation = 0;
    adapter->abi_binding.isolate_generation = 0;
}
...
DartPlantStatus VmAdapterProveCapability(DartPlantVmAdapter* adapter,
    ...
    if (adapter == nullptr || out_proof == nullptr ||
        ...
        evidence.kind > DARTPLANT_VM_EVIDENCE_EXCEPTION_BRIDGE ||
    ...
    if (evidence.kind == DARTPLANT_VM_EVIDENCE_EXCEPTION_BRIDGE &&
        out_proof->resolved_target == 0) {
        SetLastError("VM exception bridge proof did not resolve a JumpToFrame target");
...
dartplant_vm_adapter_create(const DartPlantVmAdapterCallbacks* callbacks, void* user_data,
    ...
    adapter->abi_binding.capabilities.resize(dartplant::vm_abi::CapabilityRegistrySize());
