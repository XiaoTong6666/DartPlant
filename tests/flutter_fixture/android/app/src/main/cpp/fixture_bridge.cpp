...
void ResetTypeArgumentsProofState() {
    g_type_arguments_pressure_port.store(ILLEGAL_PORT, std::memory_order_release);
...
void OnTypeArgumentsProofEnter(DartPlantInvocation* invocation, void*) {
    ...
    const DartPlantStatus parameter_before_status =
    ...
    const bool before_ok =
        ...
        descriptor.named_count == 2 && parameter_before_status == DARTPLANT_OK &&
        parameter_before.kind == DARTPLANT_VALUE_HEAP_OBJECT && parameter_before.raw != 0 &&
        vector_before_status == DARTPLANT_OK && vector_before.kind == DARTPLANT_VALUE_HEAP_OBJECT &&
    ...
    __android_log_print(
        ...
        descriptor.positional_count, descriptor.named_count, parameter_before_status,
        static_cast<unsigned>(parameter_before.kind),
        static_cast<unsigned long long>(parameter_before.raw), vector_before_status,
    ...
    const Dart_Port pressure_port = g_type_arguments_pressure_port.load(std::memory_order_acquire);
    if (pressure_port == ILLEGAL_PORT || Dart_NewSendPort_DL == nullptr ||
        ...
        __android_log_print(
            ANDROID_LOG_ERROR, kTag,
            "TypeArguments proof Dart API DL allocation unavailable port=%lld new_send_port=%p enter_scope=%p exit_scope=%p is_error=%p",
            static_cast<long long>(pressure_port), reinterpret_cast<void*>(Dart_NewSendPort_DL),
    ...
    for (uint32_t batch = 0; batch < kMaxAllocationBatches; ++batch) {
        ...
        if (!batch_ok) {
            __android_log_print(
                ANDROID_LOG_ERROR, kTag,
                "TypeArguments proof Dart API allocation failed batch=%u allocations=%llu",
        ...
        __android_log_print(
            ...
            "TypeArguments proof native after batch=%u allocations=%llu parameter=%d/%u/0x%llx vector=%d/%u/0x%llx element=%d/%u/0x%llx parameter_relocated=%u vector_relocated=%u element_relocated=%u",
            batch + 1, static_cast<unsigned long long>(allocation_count), parameter_after_status,
            static_cast<unsigned>(parameter_after.kind),
            static_cast<unsigned long long>(parameter_after.raw), vector_after_status,
            ...
            static_cast<unsigned>(parameter_relocated), static_cast<unsigned>(vector_relocated),
    ...
    __android_log_print(
        passed ? ANDROID_LOG_INFO : ANDROID_LOG_ERROR, kTag,
        "TypeArguments proof native result dart_api=%u calls=%llu parameter_relocated=%u vector_relocated=%u element_relocated=%u before=0x%llx after=0x%llx passed=%u",
...
DartPlantStatus InstallTypeArgumentsProofHook(Dart_Handle retained_closure) {
    ...
    const DartPlantStatus evidence_status = dartplant_runtime_register_compiler_abi_evidence(
        g_runtime, g_type_arguments_closure, &kDartPlantTypeArgumentsClosureAbiEvidence);
    __android_log_print(evidence_status == DARTPLANT_OK ? ANDROID_LOG_INFO : ANDROID_LOG_ERROR,
                        kTag, "TypeArguments closure compiler evidence status=%d error=%s",
                        evidence_status,
                        evidence_status == DARTPLANT_OK ? "none" : dartplant_last_error());
...
DartPlantStatus InstallAdvancedOrdinaryAotHooks() {
    if (g_verified_abi_double_hook != nullptr || g_verified_abi_double_observer_hook != nullptr) {
        return DARTPLANT_ALREADY_HOOKED;
    }
    if (g_verified_abi_double == nullptr &&
    ...
    return ordinary_aot_discovery
               ? DARTPLANT_OK
               : (ordinary_hook_status != DARTPLANT_OK ? ordinary_hook_status
                                                       : DARTPLANT_UNSUPPORTED_ABI);
}
...
void CompleteBootstrap(DartPlantStatus bootstrap_status, DartPlantLiveVmBootstrapInfo bootstrap,
    ...
    // shortly install a physical hook on verifiedAbiDouble; a runtime that has
    // never validated the artifact must not accept already-patched code as the
    // compiler fingerprint source of truth.
    DartPlantMethod* artifact_prime = nullptr;
    const DartPlantMethodQuery artifact_prime_query = {
        .struct_size = sizeof(DartPlantMethodQuery),
        .library_uri = "package:dartplant_fixture/main.dart",
        .class_name = "Global",
    ...
    const DartPlantStatus artifact_prime_status =
        dartplant_runtime_find_method(g_runtime, &artifact_prime_query, &artifact_prime);
    if (artifact_prime != nullptr) dartplant_release_method(artifact_prime);
    __android_log_print(
        artifact_prime_status == DARTPLANT_OK ? ANDROID_LOG_INFO : ANDROID_LOG_ERROR, kTag,
        "advanced artifact prebind status=%d error=%s", artifact_prime_status,
        artifact_prime_status == DARTPLANT_OK ? "none" : dartplant_last_error());
    if (artifact_prime_status != DARTPLANT_OK) {
        g_cold_bootstrap_status.store(artifact_prime_status, std::memory_order_release);
        return;
    }
...
dartplant_fixture_simple_facade_install() {
    return dartplant_simple_consumer_install();
}

extern "C" __attribute__((visibility("default"))) uint64_t
dartplant_fixture_simple_facade_stage1() {
    return dartplant_simple_consumer_stage1();
}

extern "C" __attribute__((visibility("default"))) uint64_t
dartplant_fixture_simple_facade_stage2() {
    return dartplant_simple_consumer_stage2();
}
...
dartplant_fixture_type_arguments_proof_prepare_mode(Dart_Handle retained_closure,
    ...
    Dart_Port pressure_port = ILLEGAL_PORT;
    const Dart_Handle port_status = Dart_SendPortGetId_DL(pressure_send_port, &pressure_port);
    if (port_status == nullptr || Dart_IsError_DL(port_status) || pressure_port == ILLEGAL_PORT) {
        return DARTPLANT_INVALID_ARGUMENT;
    }
    g_type_arguments_pressure_port.store(pressure_port, std::memory_order_release);
    const DartPlantStatus host_status = dartplant_fixture::InstallLocalGateHost();
    if (host_status != DARTPLANT_OK) return host_status;
...
extern "C" __attribute__((visibility("hidden"))) int dartplant_fixture_initialize_with_registers(
    ...
    for (uint32_t index = 0; index < descriptor_count; ++index) {
        const auto* descriptor = dartplant_flutter_vm_descriptor_at(index);
        if (descriptor != nullptr && descriptor->snapshot_hash != nullptr &&
            std::strcmp(descriptor->snapshot_hash, snapshot_info.snapshot_hash) == 0) {
            matched_descriptor = descriptor;
            break;
        }
    }
    const bool profile_gate = descriptor_count == 3 && matched_descriptor != nullptr &&
                              dartplant_flutter_vm_descriptor_at(descriptor_count) == nullptr;
    __android_log_print(
        profile_gate ? ANDROID_LOG_INFO : ANDROID_LOG_ERROR, kTag,
        "DartPlant Flutter VM adapter profile gate: %u count=%u dart=%s flutter=%s",
        static_cast<unsigned>(profile_gate), static_cast<unsigned>(descriptor_count),
        matched_descriptor == nullptr || matched_descriptor->dart_version == nullptr
            ? "none"
            : matched_descriptor->dart_version,
        matched_descriptor == nullptr || matched_descriptor->flutter_version == nullptr
            ? "none"
            : matched_descriptor->flutter_version);
    if (!profile_gate) {
        g_cold_bootstrap_status.store(DARTPLANT_PROFILE_MISMATCH, std::memory_order_release);
        return DARTPLANT_PROFILE_MISMATCH;
    }
    ...
    const uint64_t kExpectedCreateCapabilities = dartplant::vm_abi::ColdRequiredCapabilityMask();
    ...
    const uint64_t kExpectedVerifiedCreate = dartplant::vm_abi::VerifiedAfterCreateCapabilityMask();
    ...
    const bool generation_gate =
        ...
        verified_quiesced == kExpectedVerifiedCreate && verified_invalidated == 0 &&
